...
## Motivation
#### The Problem
Let's say we want to build a graphing calculator. This mean we'd like to visualize various mathematical
functions. For example, consider the function `y = sin(x)`. Let's take look at this function in the graph below:

<Sinx client:visible />

You can play around with the function to see how it changes with frequency and resolution. Try decreasing the
resolution all the way down and see how it gets janky. Now max-out the resolution, and try increasing the frequency.
You'll notice how the function gets more "hilly" and complex. Now turn down both of them, and see how the function
doesn't even look close to the actual sin function.

Let's explore why this happens, and then we'll introduce Marching Squares to solve this issue.
